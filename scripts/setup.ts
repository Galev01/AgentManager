...
async function main() {
  ...
  const runtimes: any[] = [{
    ...
    notes: "Default OpenClaw runtime. Edit this file or rerun `pnpm bootstrap` to change.",
  }];
  if (answers.hermesEnabled) {
    runtimes.push({
...
function renderBridgeEnv(v: {
  ...
  return `# Generated by \`pnpm bootstrap\`. Edit freely; rerun bootstrap to regenerate.
BRIDGE_HOST=${v.host}
BRIDGE_PORT=${v.port}
