...
} from "../services/claude-code-ask.js";
...
async function resolveHermesRuntimeId(
  registry: RuntimeRegistry,
  runtimeConfig: RuntimeConfigService,
): Promise<string | null> {
  const all = await registry.list();
  const match = all.find((d) => d.kind === "hermes" && (d.enabled ?? true));
...
function registerHermesSayRoute(target: ExpressRouter, deps: ClaudeCodeRouterDeps): void {
  target.post("/claude-code/hermes-say", async (req, res) => {
    ...
    const hermesRuntimeId = await resolveHermesRuntimeId(deps.registry, deps.runtimeConfig);
    if (!hermesRuntimeId) {
      console.warn("[claude-code] hermes_say rejected: could not resolve Hermes runtime id");
      return res.status(503).json({ error: "hermes runtime not configured" });
    }
...
export function createClaudeCodeRouter(deps: ClaudeCodeRouterDeps): ExpressRouter {
  ...
  registerHermesSayRoute(router, deps);
  ...
  router.post("/claude-code/ask", async (req, res) => {
    ...
    try {
      ...
      // Phase D: structured 409 for capability-gated rejections (Hermes etc).
      if (err instanceof ClaudeCodeUnsupportedRuntimeError) {
        return res.status(409).json({
          ok: false,
          error: {
            code: "UNSUPPORTED_CAPABILITY",
            runtimeId: err.runtimeId,
            capabilityId: "claudeCode.ask",
            reason: err.reason,
            message: err.message,
          },
        });
      ...
      if (/unsupported/i.test(message)) return res.status(409).json({ error: message });
  ...
  router.post("/claude-code/sessions/:id/summarize", async (req, res) => {
    ...
    // Load the session record so we can pass runtimeId + agentName to summarize.
    const sessions = await listSessions(config.claudeCodeSessionsPath);
...
// Back-compat default export: used only if legacy imports remain in server.ts.
// The preferred usage is createClaudeCodeRouter(deps).
