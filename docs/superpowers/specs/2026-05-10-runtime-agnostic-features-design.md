...
## Phase A2.x addendum — finishing the migration (implemented)

After the initial Phase A2 landed (catalog reads + `/runtimes/health`), a 2026-05-10 Codex adversarial review found seven write-path / worker surfaces that still hit `callGateway` directly. Phase A2.x closes those gaps. See `docs/superpowers/plans/2026-05-10-runtime-agnostic-finish-migration.md` for task-by-task detail. Highlights:

- **Contract additions (Wave 1).** `RuntimeActionId` extends with `sessions.{create,reset,abort,compact,delete}` + `cron.run`. `RuntimeReadCapabilityId` extends with `sessions.usage`, `cron.status`, `tools.effective`. `RuntimeAdapter.read?(capabilityId, params)` is added for per-capability reads outside `listEntities`. `sessions.send` payload gains `awaitCompletion?: boolean` + `timeoutMs?: number`; when `true`, the adapter waits for terminal status and returns `{ assistantText, elapsedMs, sessionKey }` so background workers can be runtime-neutral. OpenClaw adapter implements all of the above and absorbs the JSONL session-tail logic; Hermes / Zeroclaw / Nanobot adapters declare unsupported (Hermes additionally serves `sessions.send` synchronously via `/v1/chat`).
- **Bridge-side resource indexes.** `cron-store` (`apps/bridge/data/cron-jobs.json`) and `agent-sessions-index` (`apps/bridge/data/agent-sessions-index.json`) map `{ id → runtimeId, agentName }` so existing-resource operations resolve their stored runtime via `resolveRuntimeForResource`. Pre-existing entries without a stored `runtimeId` fall back to primary for back-compat. YouTube chat sessions, codebase-review project records, and Claude Code sessions gain optional `runtimeId` + `agentName` (and `runtimeSessionKey` for Claude Code) on their persisted records.
- **Workers dispatch via adapter.** YouTube chat worker and the codebase-reviewer runner call `adapter.invokeAction("sessions.create", …)` then `adapter.invokeAction("sessions.send", { awaitCompletion: true })` instead of `callGateway` + JSONL tail. Both use a module-singleton DI configured at server boot (`configureYoutubeChatWorker`, `configureCodebaseReviewerWorker`).
- **Models endpoint correctness.** `service.readCatalog({ runtimeId })` honors the resolved runtime end-to-end so `/models?runtimeId=hermes` no longer returns the primary runtime's catalog stamped with `runtimeId: "hermes"`.
- **Claude Code orchestrator (partial migration).** Sessions carry an optional `runtimeId`. The `"openclaw"` literal id keeps the legacy `callGateway` flow (idempotency keys, baseline polling, JSONL tail). Other runtimes establish a `runtimeSessionKey` via `sessions.create` and dispatch each turn via `sessions.send` with `awaitCompletion: true`. Summarize gracefully returns `null` when the runtime adapter doesn't support `sessions.send`. **Future-reuse:** generalizing the OpenClaw branch behind a `claudeCode.ask` action so the orchestrator is fully runtime-agnostic — four Phase-D tests in `claude-code-orchestrator-runtime.test.ts` are marked `test.skip` against this refactor.
